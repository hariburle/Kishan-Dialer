...
- [x] **Popular Contact Favoriting Nickname Lookup**: Automatically query `deviceContacts` when favoriting a popular item to resolve and store its custom `nickname` on `FavoriteContact`.
- [x] **Dual Dialers Across All Favorite Card Styles**: Enabled side-by-side dual dialers (`Phone` and `WhatsApp`) in Ask & Learn / Ask Always mode across Bento, Grid, and Material card styles.
- [x] **Keypad Hybrid Dial Buttons Design Overhaul**: Redesigned Keypad action row with wide, high-visibility Hybrid Action Buttons (`[ 📞 Phone Call ]` and `[ 💬 WhatsApp ]`) with distinct brand colors (`#059669` and `#25D366`) and glowing preferred borders.
